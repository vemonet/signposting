...
class MediaType(str):
    ...
    **not** supported by this class, as they do not form part of the
    ...
    def __new__(cls, value=str):
        ...
        if not match:
            raise ValueError(
                f"Media type invalid according to RFC6838: {value}")
        ...
        if main not in cls.MAIN:
            warn(f"Unrecognized media type main tree: {main}")
        # Ensure we use the matched string
        t = super().__new__(cls, match.group())
        t.main = main
        t.sub = sub
        return t
...
SIGNPOSTING = {l.value for l in LinkRel}
...
class Signpost:
    ...
    def __init__(self,
                 rel: LinkRel | str,
                 target: AbsoluteURI | str,
                 media_type: MediaType | str | None = None,
                 profiles: AbstractSet[AbsoluteURI] | str | None = None,
                 context: AbsoluteURI | str | None = None,
                 link: Link = None):
    ...
    def __eq__(self, o: object) -> bool:
        ...
        return all(a == b for a, b in zip(self._eq_attribs(), o._eq_attribs()))
    ...
    def with_context(self, context: AbsoluteURI | str | None) -> Signpost:
...
class Signposting(Iterable[Signpost], Sized):
    ...
    @property
    def context_url(self) -> AbsoluteURI | None:
    ...
    def for_context(self, context:AbsoluteURI | str | None) -> Signposting:
    ...
    def __iter__(self) -> Iterator[Signpost]:
        ...
        yield from self.describedBy
        yield from self.items
        yield from self.linksets
    ...
    def _repr_signposts(self, signposts) -> str:
        ...
        return " ".join({d.target for d in signposts})
    ...
    def __add__(self, other: Signposting | Iterable[Signpost]) -> Signposting:

"""Test data from Benchmarks for Apples-to-Apples FAIR Signposting

Adapted from https://w3id.org/a2a-fair-metrics/ under the terms
of Creative Commons Zero v1.0 Universal
<https://spdx.org/licenses/CC0-1.0>

except 02-html-full.index.html

"Apples to Apples Dataset"
by Josiah Carberry (not really)
<https://w3id.org/a2a-fair-metrics/02-html-full/>

Redistributed under the terms of
Creative Commons Attribution 4.0 International License
<http://creativecommons.org/licenses/by/4.0/>
"""

